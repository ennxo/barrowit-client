...
import { getArchivedAccount } from '../../api'
import { ArrowUturnLeftIcon } from '@heroicons/react/24/solid'
import { useState } from 'react'
import { ImageModal } from '../../../../components/Modal'
import { BeatLoader } from 'react-spinners'

export const ViewAccountsArchived = () => {
  const { id } = useParams()
  const { data: account, hasPending } = useFetch(getArchivedAccount(id))
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">
              <div className="divide-y divide-gray-200 rounded-md border border-gray-200">
                <div className="flex items-center justify-between py-3 pl-3 pr-4 text-sm">
                    ...
                    <img src={account.valid_id} alt="asset-image" className="w-full h-60"/>
                    </Link>
                </div>
              </div>
            </dd>
