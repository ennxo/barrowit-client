...
export const CompleteModal = ({ open, setOpen, account, navigate, transactionIds }) => {
  const cancelButtonRef = useRef(null)
  const [error, setError] = useError()

  const onComplete = async () => {
    try {
      await completeRequest(account.id, transactionIds)
      navigate("/reservations/completed")
      setError({})
      setOpen(false)
    } catch (error) {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    >
                      Approve {account.first_name + " " + account.last_name}'s'
                      requests
                    </Dialog.Title>
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        ...
                        {account.first_name + " " + account.last_name}'s
