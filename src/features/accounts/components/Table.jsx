...
import Select from 'react-select'
  ...
  const filtered = (array, query, selectedAccounts) => {
    const accounts = array?.filter(account => account?.User?.roles === "user")
    return accounts.reduce((filteredArray, account) => {
      if(query && !account.full_name.toLowerCase().includes(query.toLowerCase())) {
        return filteredArray
      }
      if(selectedAccounts.length > 0 && !selectedAccounts.includes(parseInt(account.User.isVerified))) {
        return filteredArray
      }
      return [...filteredArray, account]
    }, [])
  }
...
export const Table = () => {
    ...
    //Selected Account 
    const [selectedAccounts, setSelectedAccounts] = useState([0,1])
    //Checkbox
    const [selectedItems, setSelectedItems] = useState([])
    const [selectedAll, setSelectedAll] = useState(false)
    // New Data
    const users = filtered(data, query, selectedAccounts)
    console.log(users)
     //Pagination
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-10">
        <div className="sm:flex sm:items-center">
          <div className="sm:flex-auto">
            <h1 className="text-xl font-semibold text-gray-900">Resident's Accounts</h1>
            <p className="mt-2 text-sm text-gray-700">
              A list of all the resident account.
            </p>
          </div> 
          ...
          <div className="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
              <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                    ...
                    {accounts?.map((account) => (
                      <tr key={account.id}>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          <span className={cn("inline-flex rounded-full bg-gray-100 px-2 text-xs font-semibold leading-5 text-gray-800", {"bg-orange-100 text-orange-800" : !!parseInt(account.User.isVerified)})}>
                            {!!parseInt(account.User.isVerified) ? 'Verified' : 'Unverified'}
                          </span>
                        </td>
                ...
                <EmptyData array={accounts} message={"No resident's accounts found"} hasPending={hasPending} />
              </div>
            </div>
          </div>
        </div>
