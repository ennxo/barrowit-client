...
export const ReturningTable = () => {
    const { data, hasPending } = useFetch(getRequests("approved"))
    //Filter assets that returning on current date
    ...
    return ( 
      <Layout title="Returning/Pick Up Assets" description="Assets that should be returned on time.">
                    ...
                    {accounts?.map((account) => (
                      <tr key={account.id}>
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 space-x-5">
                          <Link to={`/reservations/approved/details/${account.id}/returning`} className="text-blue-600 hover:text-blue-900">
